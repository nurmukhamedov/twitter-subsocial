...
export const signAndSendTx = async (tx: any) => {
	const { isWeb3Injected, web3Enable, web3AccountsSubscribe, web3FromAddress } = await import(
...
export const postTweet = async (tweet: string) => {
	...
	const cid = await ipfs.saveContent({
		title:
			selectedProfile && selectedProfile.content?.name
				? selectedProfile.content?.name
				: selectedAddress,
		body: tweet,
